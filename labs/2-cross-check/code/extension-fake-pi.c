...
enum { max_trace = 128 };
unsigned ntrace = 0;
static unsigned trace_on_p = 0;
#define trace(msg, args...) do {                                        \
    if(trace_on_p) {                                                    \
        output("TRACE: " msg, ##args);                                   \
        if(ntrace++ >= max_trace) {                                     \
...
unsigned gpio[7] = {
    GPIO_BASE + 0x00, // gpio_fsel0
    GPIO_BASE + 0x04, // gpio_fsel1
    GPIO_BASE + 0x08, // gpio_fsel2 
    GPIO_BASE + 0x0c, // gpio_fsel3 
    GPIO_BASE + 0x1C, // gpio_set0
    GPIO_BASE + 0x28, // gpio_clr0
    GPIO_BASE + 0x34  // gpio_lev0
};


// the value for each location.
static unsigned gpio_v[7];
