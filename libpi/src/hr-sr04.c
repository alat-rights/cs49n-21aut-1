...
int read_while_eq(int pin, int v, unsigned timeout) {
    ...
    while (1) {
        if (gpio_read(pin) != v)
            return 1;
        if ((timer_get_usec() - startTime) >= timeout)
            return 0;
    }
}
...
int hc_sr04_get_distance(hc_sr04_t h, unsigned timeout_usec) {
    unsigned trigger = h.trigger;
    unsigned echo = h.echo;
    gpio_set_on(trigger);
    delay_us(15);
    gpio_set_off(trigger);  // Set echo to 5 V
    if (!read_while_eq(echo, 0, timeout_usec)) {
        delay_us(15);
        return -1;
    }
    unsigned startTime = timer_get_usec();
    if (read_while_eq(echo, 1, timeout_usec)) {  // Wait for echo to become 0V
        unsigned currTime = timer_get_usec();
        delay_us(15);
        return ((currTime - startTime) / 148);
    }
    else {
        delay_us(15);
        return -1;
